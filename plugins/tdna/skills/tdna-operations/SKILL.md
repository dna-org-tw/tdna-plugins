...
- 缺專案／收款資料先呼叫 `expense_prepare`。工具自動建立可建立的專案、預填收款人安全表單；多筆符合或必要欄位缺漏才問使用者。不在對話蒐集銀行／身分資料，亦不以假資料建立收款人。
...
- 使用者填完安全表單後接續準備。更新既有差旅先 `records_get(module="trips")`，保留完整舊明細，帶 `trip_id` 與 `expect_updated`；不要丟棄前次憑證或重複新增同一收據。
