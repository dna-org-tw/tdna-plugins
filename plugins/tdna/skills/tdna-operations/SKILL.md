...
7. 金額與日期使用已確認資料。收款資料用 `recipients_upsert`，有類型、姓名、Email 即可先建檔，其餘欄位分次補齊（`missing` 列缺項，`complete=false` 不能送件）；僅詢問缺項、不用假資料佔位、不重述完整身分證或銀行帳號；附件用 `attachments_upload` 傳送 Base64，無需開啟網頁。
